...
Ext.onReady(function(){
    ...
    Ext.define('Admin.basic.searchForm.gameQuest',{
    ...
    Ext.define('Admin.basic.grid.gameQuest',{
        extend:'Admin.basic.grid',
        columns:[
            {text:'游戏类型',dataIndex:'game',flex:1},
            {text:'表单问题',dataIndex:'table',flex:1}
        ],
        store:store,
        bbar: {
            xtype: 'pagingtoolbar',
        ...
        },'-',
        {
            ...
            text:'编辑',
            handler : function(){
                var rows = gameQuestGrid.getSelection();
                if(rows.length != 1){
                    tip('请选择要编辑的数据！');
                    return;
                }
    ...
    //搜索块
    var gameQuestSearchForm = new Admin.basic.searchForm.gameQuest();
    gameQuestSearchForm.render(Ext.getBody());
    //列表
    var gameQuestGrid = new Admin.basic.grid.gameQuest();
    gameQuestGrid.render(Ext.getBody());
    //初始化数据集
    storeReload(store,1);	
});
</script>