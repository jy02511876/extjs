...
Ext.onReady(function(){
    ...
    Ext.define('Admin.basic.searchForm.level',{
    ...
    Ext.define('Admin.basic.grid.level',{
        ...
        },'-',
        {
            ...
            text:'编辑',
            handler : function(){
                var rows = levelGrid.getSelection();
                if(rows.length != 1){
                    tip('请选择要编辑的数据！');
                    return;
                }
                ...
                Ext.Ajax.request({
                    ...
                    success:function(response){
                        ...
                        if(data.success){
                            ...
                            editForm.findField('level').setValue(data.data.level);
    ...
    //搜索块
    var levelSearchForm = new Admin.basic.searchForm.level();
    levelSearchForm.render(Ext.getBody());
    //列表
    var levelGrid = new Admin.basic.grid.level();
    levelGrid.render(Ext.getBody());
    //初始化数据集
    storeReload(store,1);	
});
</script>