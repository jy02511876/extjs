...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.Store',{
        fields:['id','game','game_id','times','timesDesc'],
        pageSize:20,
        proxy: {
            type: 'ajax',
    ...
    Ext.define('Admin.basic.searchForm.returnTimes',{
    ...
    Ext.define('Admin.basic.grid.returnTimes',{
        extend:'Admin.basic.grid',
        columns:[
            {text:'游戏类型',dataIndex:'game',flex:1},
            {text:'装备找回次数限制',dataIndex:'timesDesc',flex:1}
        ],
        store:store,
        bbar: {
            xtype: 'pagingtoolbar',
        ...
        },'-',
        {
            ...
            text:'编辑',
            handler : function(){
                var rows = returnTimesGrid.getSelection();
                if(rows.length != 1){
                    tip('请选择要编辑的数据！');
                    return;
                }
                ...
                Ext.Ajax.request({
                    ...
                    success:function(response){
                        ...
                        if(data.success){
                            ...
                            editForm.findField('times').setValue(data.data.times);
    ...
    //搜索块
    var returnTimesSearchForm = new Admin.basic.searchForm.returnTimes();
    returnTimesSearchForm.render(Ext.getBody());
    //列表
    var returnTimesGrid = new Admin.basic.grid.returnTimes();
    returnTimesGrid.render(Ext.getBody());
    //初始化数据集
    storeReload(store,1);	
});
</script>